...
export interface MockTitleSeed {
  ...
  // The ACTUAL Friday this title's release week starts (ISO "YYYY-MM-DD").
  // Critical for freshness: without this, a curated title recomputes its
  // release date against whatever week the app happens to be showing,
  // making it look like a brand-new release forever, every week, no
  // matter how much real time has passed. This anchors each seed to the
  // one real week it belongs to, so it naturally ages out afterward.
